...
public class JsonPluginsUtil {
	...
	public static String dynaBean2Json(DynaBean bean) {
		
		/*replace("{}", "null")		空对象 会转换成{}	 修改 将 {} 替换成 null									*/
		/*replace("\"[]\"", "[]") 邵斌20140811修改	android解析有"[]"问题	需将	"[]" 替换成  []										*/
		return JSONSerializer.toJSON(bean).toString().replace("{}", "null").replace("\"[]\"", "[]");
...
//	                new DateJsonValueProcessor(datePattern));   
	...
	public static String dynaBeanList2Json(List<DynaBean> beans) {
		...
		for (int i = 0; i < size; i++) {
			...
			rest.append(dynaBean2Json(beans.get(i))+((i<size-1)?",":""));
	...
	/**
	 * 将json转化成动态bean[用 morphDynaBean.get(""); 获取属性对应的值]
	 * @param json
	 * @return
	 */
	public static DynaBean json2bean(String json){
		
		JSON jo = JSONSerializer.toJSON(json);
		MorphDynaBean morphDynaBean = (MorphDynaBean) JSONSerializer.toJava(jo);//MorphDynaBean
...
//        o = JSONSerializer.toJava(jsonObject);//MorphDynaBean
	...
	@SuppressWarnings("unchecked")
	public static List<DynaBean> json2arrayBean(String json){
		
		JSON jo = JSONSerializer.toJSON(json);
		List<DynaBean> dynaBean = (List<DynaBean>) JSONSerializer.toJava(jo);//MorphDynaBean
	...
	public static void main(String[] args) {
		...
		_dynaBean.set("nulldate", new Date());
		_dynaBean.set("nulldate", null);
		_dynaBean.set("nullStr", null);
		_dynaBean.set("str", "");
		

		_dynaBean.get("nulldate");
		_dynaBean.get("nullStr");
		_dynaBean.get("str");
		
		System.out.println(JsonPluginsUtil.dynaBean2Json(_dynaBean));
