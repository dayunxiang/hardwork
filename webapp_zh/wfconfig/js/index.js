...
// 设置高度
 function height() {
   ...
   $("svg").css({"height":(h-80)+"px","width": (w- $(".option").width() - $(".sideBar").width()) + 50 + "px"})
...
// ------------------ 方法定义 -----------------------
 ...
 function findWorkFlowCfgList() {
  ...
  function newWorkFlow() {
   ...
   function ReNameAndCopy() {
     ...
     * @return {[type]}   [description]
     */
     $(".wfCfg").off('click').on("click", function () {
       // 工作流id
       ...
       $(this).css("background","#CDDFF6")
       $(this).find('input').css("background","#CDDFF6")
       $(this).siblings('li').css("background","#fff")
       $(this).siblings('li').find('input').css("background","#fff")
       // 隐藏操作面板
       $(".option").css('transform', 'translateX('+(width + 80)+'px)')
       // 将工作流模板key赋值到全局变量
       currentWfKey = wfKey;
      ...
      * @return {[type]}   [description]
      */
     $(".editCfg").off('click').on("click", function (event) {
       // 阻止冒泡事件
    ...
    * @return {[type]}   [description]
    */
    $(".copyCfg").off('click').on("click", function (event) {
      // 阻止冒泡事件
      event.stopPropagation();
      // 工作流id
      var wfId = $(this)[0].dataset.wfid;
    ...
    */
    ...
    /**
     * [putDta2SelectedInput 根据元素和数据源，给对应的选择框配置数据源]
     * @param {[type]} executeRoles [description]
     */
    function putDta2SelectedInput(select, executeRoles) {
      $(select).html('')
      if (executeRoles) {
      ...
      for(var i=0;i<wfSwitchData.length;i++) {
        ...
        if (wfSwitch.onoff!='-1') {
          ...
          if (wfSwitch.nodeid=='isAssigned') {
            ...
            for(var j=0;j<wfNodeCfgData.length;j++) {
              var nodeCfg = wfNodeCfgData[j];
              // 执行中
              if (nodeCfg.nodeid=='executing'&&nodeCfg.doid=='submit'&&nodeCfg.roles!='-1') {
                zxzpz.style.display = 'block'
                zxztjpd.style.display = 'block'
              ...
              // 延期审批
              if (nodeCfg.nodeid=='delayApprove'&&nodeCfg.doshowname=='通过'&&nodeCfg.roles!='-1') {
                yqsppz.style.display = 'block'
              }
            }
    ...
    /**
     * [setExecutingPerson 设置执行中的人员下拉框]
     */
    function setExecutingPerson() {
      // 如果执行改派节点已经配置了接单人，则跟执行中的人员设置数据源
      for(var i=0;i<wfNodeCfgData.length;i++) {
        var nodeCfg = wfNodeCfgData[i];
        if (nodeCfg.nodeid=='implementAndReassignment' && nodeCfg.doid=='implement' 
          && nodeCfg.doshowname=='接单' && nodeCfg.roles!='-1') {
          putDta2SelectedInput('.zxz .chzn-select', nodeCfg.roles);
          // 先配置数据源，再配置选中项
          putDta2SelectedInput('#dl_chose6', nodeCfg.roles);
          chose_mult_set_ini('#dl_chose6', nodeCfg.roles);
        }
      }
    }
    /**
     * [judgeIsComplete 判断工作流配置是否完整？]
     * @return {[type]} [description]
     */
    function judgeIsComplete() {
      // 可以点击到提交这个按钮的时候，大部分已经配置完毕
      // 只要判断指派和指派是否需要审批两个节点是否配置即可
      if (wfNodeCfgData) {
        for(var i=0;i<wfNodeCfgData.length;i++) {
          var cfg = wfNodeCfgData[i];
          if (cfg.nodeid=='assigned' && cfg.roles=='-1') {
            alert('指派节点还未进行配置！');
            return;
          }
        }
      ...
      if (wfSwitchData) {
        for(var j=0;j<wfSwitchData.length;j++) {
          var switchCfg = wfSwitchData[j];
          if (switchCfg.nodeid=='isAssignNeedApprove' && switchCfg.onoff=='-1') {
            alert('指派是否需要审批节点还未进行配置！');
            return;
          }
        }
      }
 ...
 //保存退出
 $(".modalbc").off('click').on("click", function () {
