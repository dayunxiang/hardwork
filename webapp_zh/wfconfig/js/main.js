...
$("#dl_chose5").on("change", function() {
    debugger
    var vale = getJsonFromArray($(this).val());
    if (vale!='') {
        // 先配置数据源，再配置选中项
        putDta2SelectedInput('#dl_chose6', vale);
        chose_mult_set_ini('#dl_chose6', vale);
    }
});
//执行或改派
$("#qyzjdgn").find("input").on("change", function() {
    ...
    if ($("#qyzjdgn").find("input").is(':checked')) {
        ...
        if (jieDanPerson!='') {
            // 先配置数据源，再配置选中项
            putDta2SelectedInput('#dl_chose6', jieDanPerson);
            chose_mult_set_ini('#dl_chose6', jieDanPerson);
        }
    } else {
        // 取消选中
        $(".zd").css("display", "none");
        ...
        // 将转单文本框清空
        chose_mult_set_ini('#dl_chose6', '');
        putDta2SelectedInput('#dl_chose6', '');
...
$("#nextd").off('click').on("click", function() {
    ...
    var callBackFunD = function(res) {
        if (res.resCode==1) {
            ...
            putDta2SelectedInput('.zxz .chzn-select', jieDanPerson);
            // 样式控制
