...
function findRegionList() {
	//获取图片信息
		JQueryUtil.sendAsyncRCP(pathUrl + '/configManage/regionCfg/findRegionCfgByTypeId', {
			regionTypeId: ''
		},
		function success(result) {
			if (result.data) {
				localStorage.setItem('regionListJson',result.data);
			}
		},
		function error(error) {
			console.log(error.msg);
...
function getRegionNameById(regionId){
	var value = localStorage.getItem('regionListJson');
	if (value==''||value==null) {
		findRegionList();
		value = localStorage.getItem('regionListJson');
	}
	if(value){
		var regionList = JSON.parse(value);
	    for (var i=0;i<regionList.length;i++) {
	        if (regionList[i].id == regionId) {
	            return regionList[i].quyuname;
	        }
	    }
	    return regionId;
    }
